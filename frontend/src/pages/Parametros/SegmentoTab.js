...
const SegmentoTab = () => {
  ...
  const fetchItems = async () => {
    setLoading(true);
    try {
      const { data } = await api.get("/parametros/segmento", {
        params: { searchParam },
      });
      setItems(data);
    } catch (error) {
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingItem) {
        await api.put(`/parametros/segmento/${editingItem.id}`, formData);
        toast.success("Segmento atualizado com sucesso!");
      } else {
        await api.post("/parametros/segmento", formData);
  ...
  const handleDelete = async (itemId) => {
    if (window.confirm("Tem certeza que deseja excluir?")) {
      setLoading(true);
      try {
        await api.delete(`/parametros/segmento/${itemId}`);
  ...
  return (
    <Box>
      ...
      <Dialog open={openDialog} onClose={handleCloseDialog}>
        <DialogTitle>
          {editingItem ? "Editar Segmento" : "Novo Segmento"}
        </DialogTitle>
