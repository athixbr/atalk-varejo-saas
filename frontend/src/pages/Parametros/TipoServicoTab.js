...
const TipoServicoTab = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingItem) {
        await api.put(`/tipo-servico/${editingItem.id}`, formData);
        toast.success("Tipo de Serviço atualizado com sucesso!");
      } else {
        await api.post("/tipo-servico", formData);
  ...
  const handleDelete = async (itemId) => {
    if (window.confirm("Tem certeza que deseja excluir?")) {
      setLoading(true);
      try {
        await api.delete(`/tipo-servico/${itemId}`);
